...
def extract_dollarsign(input_texts, output_file, pre_pad_num=100, post_pad_num=100, maxlen=20):
    """ Extract the sentences containing weight units

    @return: the number of instances found
    """

    ins_count = 0
    reg_exp = ".{" + str(pre_pad_num) + "}(?: kg)|(?: lbs)|(?: pounds)|(?: kilograms)|(?: ounces)|(?: gallons).{" + str(post_pad_num) + "}"
    ...
    for res in results:
        ...
        for sent in doc.sents:
            ...
            if (" kg" in plain_text) or \
               (" lbs" in plain_text) or \
               (" pounds" in plain_text) or \
               (" kilograms" in plain_text) or \
               (" ounces" in plain_text) or \
               (" gallons" in plain_text):
