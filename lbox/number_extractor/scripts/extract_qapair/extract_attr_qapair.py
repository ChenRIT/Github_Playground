...
def extract_sents(input_texts, output_file, post_pad_num=100):
    ...
    reg_exp = r"cost.{,20}\?.{" + str(post_pad_num) + "}"
    ...
    for res in results:
        ...
        for i in range(len(sents)):
            if 'cost' in sents[i].text and \
               '?' in sents[i].text and \
               i+1 < len(sents):
                next_sent = sents[i+1]
                #search_num = re.search(r"[0-9.,]+[0-9]", next_sent.text)
                # if search_num:
                if "$" in next_sent.text or "dollar" in next_sent.text:
                    has_num = False
        ...
        for fname in all_files:
            if "warc.gz.txt" in fname:
                with open(fname, "r", errors='replace') as ifile:
                    doc = ifile.read()
                    ins_count += extract_sents(doc, ofile, pad_right)
